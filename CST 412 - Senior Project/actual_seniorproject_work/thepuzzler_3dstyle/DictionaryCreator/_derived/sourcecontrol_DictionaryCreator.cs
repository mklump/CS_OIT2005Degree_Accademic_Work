	...
	public class DictionaryCreator : Solution
	{
		...
		public DictionaryCreator()
		{
			...
			if( file == null )
			{
				file = new string []
				{
					@"thepuzzler_3dstyle/base_dictionary#1.txt", 
					@"thepuzzler_3dstyle/base_dictionary#2.txt",
					@"thepuzzler_3dstyle/base_dictionary#3.txt",
					@"thepuzzler_3dstyle/base_dictionary#4.txt"
				};
			}
			if( puzzle.GetUpperBound(0) > puzzle.GetUpperBound(1) &&
				puzzle.GetUpperBound(0) > puzzle.GetUpperBound(2) )
				maximumWordLength = puzzle.GetUpperBound(0);
			else if( puzzle.GetUpperBound(1) > puzzle.GetUpperBound(0) &&
				puzzle.GetUpperBound(1) > puzzle.GetUpperBound(2) )
				maximumWordLength = puzzle.GetUpperBound(1);
			else
				maximumWordLength = puzzle.GetUpperBound(2);
		}
		/// <summary>
		...
		{
			CheckDictionaryParameters( baseDictionary, dictionarySize );
			StreamReader reader = null;
			try
			{
				reader = new StreamReader( file[baseDictionary - 1] );
			}
			catch( System.IO.FileNotFoundException )
			{
				...
				catch( System.IO.FileNotFoundException error )
				{
					Trace.WriteLine( "You must make absolutely sure that the files base_dictionary#1.txt," + 
						...
						"correctly configured to be received as input to the library: " + this.ToString() + " !!" +
						"\nMessage:\n" + error.ToString() );
				}
			}
			finally{  }
		...
		/// <summary>
		/// This helper method will validate the arguements passed to CreateDictionary() that were
		/// provided by the user. The static number limitations are based solely on the known contents
		/// of each dictionary word list file.
		/// </summary>
		...
		private void CheckDictionaryParameters( int baseDictionary, int dictionarySize )
		{
