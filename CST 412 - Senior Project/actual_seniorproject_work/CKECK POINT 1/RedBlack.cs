...
namespace ns_RedBlack
{
	...
	public class RedBlack
	{
		...
		/// <summary>
		/// Property Parent (RedBlack) Read access only
		/// </summary>
		...
		/// <summary>
		/// Property Right (RedBlack) Read access only
		/// </summary>
		...
		/// <summary>
		/// Property Left (RedBlack) Read access only
		/// </summary>
		...
		/// <summary>
		/// Property Color (string)	Read access only
		/// </summary>
		public string Color
		{
			get
		...
		/// <summary>
		/// This two arguement method/operations accepts an object value and creates
		/// a new RedBlack node by first calling the default RedBlack() then
		/// inserting the resultant new node.
		/// </summary>
		...
		private RedBlack GetRoot( ref RedBlack T )
		{
		...
		public virtual bool RB_LessThanOperator( object leftArguement, object rightArguement )
		{
			...
			else if( leftArguement is char[] && rightArguement is char[] )
			{
				...
				return string.Compare(new string(x), new string(y), true) < 0; 
			}
		...
		/// while loop. If not found, the Sentinel node is returned.</returns>
		...
		private void Left_Rotate( ref RedBlack T, RedBlack x )
		{
			...
			y.left.parent = x;
			y.parent = x.parent;
			if( x.parent == Sentinel.Node )
			{
				RedBlack root = GetRoot( ref T );
				root = y;
			}
		...
		/// <param name="z">RedBlack node we're going to delete.</param>
		/// <returns>The last RedBlack node worked with.</returns>
		public RedBlack RB_Delete( ref RedBlack T, RedBlack z )
		{
			RedBlack x, y;
			if( z.left == Sentinel.Node || z.right == Sentinel.Node )
				y = z;
			else
				y = Tree_Successor( z );
		...
		{
			while( x != GetRoot( ref T ) && x.color == "black" )
			{
				if( x == x.parent.left )
				{
			...
			x.color = "black";
		} // End public RedBlack RB_Delete( RedBlack T, RedBlack z ) {}
	...
	public class RedBlack_TestClass
	{
		...
		{
			val = "Yet another one final string value.";
			root.RB_Delete( ref root, root.Iterative_Tree_Search( root, val.ToCharArray() ) );
			root.Inorder_Tree_Walk( root );
		}
		/// <summary>
		/// Test 11
