<%@ Page Language="vb" AutoEventWireup="false" Codebehind="DataReaderCode.aspx.vb" Inherits="Mod10VB.DataReaderCode" %>
...
<HTML>
  ...
  { 
	...
	if (sValue == "Dim conn As New SqlConnection('valid connection string')") {
		...
		box1 = sValue;
	} else if (sValue == "Dim dr As SqlDataReader"){ 
		arguments.IsValid = true;
		box1 = sValue;
    } else { 
        Label2.InnerHtml = ""
  ...
  { 
	var sValue = arguments.Value; 
	if (sValue == "Dim cmdAuthors As New SqlCommand('select * from Authors', conn)")
		{
		...
		{ 
			if (box1 != "Dim dr As SqlDataReader")
			{
			arguments.IsValid = true; 
			box2 = sValue;
			}
  ...
  { 
	...
	if (sValue == "Dim cmdAuthors As New SqlCommand('select * from Authors', conn)")
		{
			if (box1 == "Dim cmdAuthors As New SqlCommand('select * from Authors', conn)")
			{
				arguments.IsValid = false;
			}
			else if (box2 == "Dim cmdAuthors As New SqlCommand('select * from Authors', conn)")
			{
				arguments.IsValid = false; 
			}
			else
	...
	{ 
		...
		else if (box2 == "Dim dr As SqlDataReader")
		{
		arguments.IsValid = false;
		}
		else
