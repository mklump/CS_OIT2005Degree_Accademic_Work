							...
							)
{
	...
	*/
	...
	/*Jasens code Begins: All values are now moved to the register 0 to be returned*/
	/*Implement the entire function to ensure full integration.*/
	...
	/*Jasen Peterman: */
	/*Stmnt checks to see if func is void, if so then a return stmnt is invalid*/
	if(pCurFunction->ident_ptr.ident_info.func.func_declared_type == FUNC_DEF_DECLARED_VOID)
	{
		printf(
		"cst320 : ccomp. error file[%s] line[%d] source line[%d].\n compile error: void statement cannot return value. \n",
				__FILE__,__LINE__, lineno
				);
		assert(0) ;
	}
	/* Jasen Peterman
	 * this if then checks for a func that returns chars, if so it posts an error and exits.
	 */
	else if(pCurFunction->ident_ptr.ident_info.func.func_declared_type ==FUNC_DEF_DECLARED_CHAR)
	{
		printf(
		"cst320 : ccomp. error file[%s] line[%d] source line[%d].\n compile error: functions returning character values not supported in this language. \n",
				__FILE__,__LINE__, lineno
				);
		assert(0) ;
	}
	...
	case IDENT_DEF_CONSTANT_INT:
		pExpr->ident_ptr.ident_info.expr.expr_reg = 0;
		fprintf(
				outFile,
				"\tmov	%%r0,	$%d\n",
...
{
	condition_stack_item_t	temp;

	if( get_tos_condition_stack( &temp ) == NULL )
	{
		fprintf( outFile," Yo dummy, you're using a break outside of a while! Line: %d\n", lineno );
		printf(" Yo dummy, you're using a break outside of a while! Line: %d\n", lineno );
		assert(0);
	}
	else
	{
		fprintf( outFile,"jmp {}, label%d\n", temp._while.out_while_label );
		printf("jmp	{},	label%d\n", temp._while.out_while_label );
	}
						  ...
						  )
{
	...
	if( get_tos_condition_stack( &temp ) == NULL )
	{
		fprintf( outFile," Yo dummy, you're using a continue outside of a while! Line: %d\n", lineno);
		printf(" Yo dummy, you're using a continue outside of a while! Line: %d\n", lineno);
		assert(0);
	}
	else
		return;
